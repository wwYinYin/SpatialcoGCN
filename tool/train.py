...
def coGCN_training(adata,adj,n_celltype,device):
    ...
    with torch.no_grad():
        output = M_probs.cpu().numpy()
        return output
 
def start(adata,adj,GCN_method,seed=2022):
    ...
    if GCN_method=="coGCN":
        output=coGCN_training(adata,adj,n_celltype,device) #[189×15]
    if GCN_method=="pygGCN":
        output=pygGCN_training(adata,adj,n_celltype,device) #[189×15]
...
def pygGCN_training(adata,adj,n_celltype,device):
    in_feat=adata.shape[1]
